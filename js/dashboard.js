...
(function (window, document) {
  ...
  function renderMeaning(sessions) {
    ...
    if (!withBuild.length) {
      buildList.appendChild(el('li', { text: 'No data yet.' }));
    } else {
      ...
      [['order', 'Chosen as step 1'],
       ...
       ['removable', 'Called droppable']].forEach(function (pair) {
        ...
        if (!total) return;
        var top = votes[0];
        var asset = window.CC_ASSETS && window.CC_ASSETS.byId ? window.CC_ASSETS.byId(top.key) : null;
        var name = asset ? (asset.code + ' · ' + (asset.shortName || asset.name)) : top.key;
        buildList.appendChild(el('li', {
          text: pair[1] + ' most often: ' + name + ' — ' +
            Math.round(top.n / total * 100) + '% (' + top.n + ')'
        }));
      });
    }
